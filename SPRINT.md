...
### Tonight
- CCR-019: Inventory & equipment display — render starting equipment, armor class computation, weight/encumbrance summary
- CCR-020: Spell management on sheet — display prepared/known spells by level, spell slot tracking
...
### Maintenance (2026-04-08)
- [x] Run all tests and report failures — 725 tests pass (29 test files), 0 failures
- [x] Run lint — 0 errors, 13 pre-existing warnings (shadcn fast-refresh, useEffect deps)
- [x] Run build — succeeds (6.46s)
- [x] Code review: CharacterView extraction clean, no new smells introduced
- [x] Check for unused imports, dead code — removed getModifier and getCarryingCapacity from CharacterView (moved to components)
