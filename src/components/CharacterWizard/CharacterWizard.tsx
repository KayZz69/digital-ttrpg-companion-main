...
export const CharacterWizard = ({ onBack }: CharacterWizardProps) => {
  ...
  const getValidationOptions = () => ({
    equipmentCostGp: getCurrentEquipmentCostInGp(),
    activeStepKeys: steps.map((step) => step.key),
  });
  ...
  const canProceed = () => {
    ...
    return getStepError(currentStepDefinition.key, character, getValidationOptions()) === null;
  };

  const handleNext = () => {
    const errors = currentStepDefinition
      ? getStepErrors(currentStepDefinition.key, character, getValidationOptions())
      : ["Unknown step."];
    if (errors.length > 0) {
      toast({
  ...
  const handleComplete = () => {
    const reviewErrors = getStepErrors("review", character, getValidationOptions());
    if (reviewErrors.length > 0) {
      toast({
        title: "Character Incomplete",
        description: reviewErrors.join("; "),
        variant: "destructive",
      });
      return;
    }

    const crossStepErrors = getCrossStepErrors(character);
    if (crossStepErrors.length > 0) {
      toast({
        title: "Character Incomplete",
        description: crossStepErrors.join("; "),
        variant: "destructive",
      });
      return;
    }
